...
class PostgreSQLDtype(PGTypeFunc, Enum):
    ...
    Macaddr8 = PGTypeFunc("Macaddr8", 8, str, read_macaddr, write_macaddr)
    Macaddr = PGTypeFunc("Macaddr", 6, str, read_macaddr, write_macaddr)
    Money = PGTypeFunc("Money", -1, float, read_money, write_money)
    Numeric = PGTypeFunc("Numeric", -1, Decimal, read_numeric, write_numeric)
    Oid = PGTypeFunc("Oid", 4, int, read_oid, write_oid)
    Path = PGTypeFunc("Path", -1, tuple | list, read_path, write_path)
    Point = PGTypeFunc("Point", 16, tuple, read_point, write_point)
    Polygon = PGTypeFunc("Polygon", -1, tuple, read_polygon, write_polygon)
    Serial2 = PGTypeFunc("Serial2", 2, int, read_serial2, write_serial2)
    Serial4 = PGTypeFunc("Serial4", 4, int, read_serial4, write_serial4)
    Serial8 = PGTypeFunc("Serial8", 8, int, read_serial8, write_serial8)
    Text = PGTypeFunc("Text", -1, str, read_text, write_text)
    Time = PGTypeFunc("Time", 8, time, read_time, write_time)
    Timestamp = PGTypeFunc("Timestamp", 8, dt, read_timestamp, write_timestamp)
    Timestamptz = PGTypeFunc("Timestamptz", 8, dt, read_stamptz, write_stamptz)
    Timetz = PGTypeFunc("Timetz", 12, time, read_timetz, write_timetz)
    Uuid = PGTypeFunc("Uuid", 16, UUID, read_uuid, write_uuid)
